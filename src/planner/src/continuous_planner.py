...
class Planner:
    ...
    def generate_plan(self):
        ...
        curr_state = self.get_current_continuous_state()
        g = 0
        ...
        while len(heap) > 0:
            ...
            if curr_state in visited:
                continue
            print("f: %f, g: %f, x: %f, y: %f, theta: %f" % (f, g, x, y, theta))
            ...
            for action in actions:
                ...
                dy = new_y - y
                dx = new_x - x
                d_theta = abs(new_theta - theta)
                distance_travelled = sqrt(dy ** 2 + dx ** 2)
                new_g = g + distance_travelled + d_theta

                # Calculate new h and f
...
if __name__ == "__main__":
    ...
    # inflation_ratio = 3
    inflation_ratio = ROBOT_SIZE / resolution + 7
    ...
    if args.com:
        ...
        filename = "control_files/task2/2_maze1_%d_%d.txt" % (goal[0], goal[1])
